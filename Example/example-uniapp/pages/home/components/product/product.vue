...
<view class="recommend_goods" v-if="modes == true">
  <view v-for="(item, index) in newList" :key="index" class="goods">
      <view class="top" @tap="jumpDetails(item)" >
        <image class="cover" lazy-load="true" :src="item.imageUrls&&item.imageUrls.length>0?item.imageUrls[0].cosUrl:'/static/img/noProduct.png'" mode="scaleToFill"></image>
      </view>
      <view class="bottom">
        ...
        <view class="price">
          <text class="text1">{{item.price+'元'}}</text>
		  <text class="text2">{{'销量：'+item.saleCount}}</text>
        </view>
        <view class="goumai">
			<view class="g_left">
...
export default {
  ...
  methods: {
    ...
    jumpDetails(e) {
      uni.navigateTo({
        url: '/pages/product/index?productId='+e.productId
      });
    }
  }
};
